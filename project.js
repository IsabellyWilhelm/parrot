...
let row_cards = document.querySelector(".cards");
...
for(let i = 0; i < number_of_cards; i++) {
    row_cards.innerHTML = row_cards.innerHTML + `
        <div class="card" data-identifier="card" onclick="game_card_select(this)">
            <div class="game-card front face" data-identifier="front">
                <img class="game-card-img" src="./assets/img/front.png">
            </div><!--front-->
            <div class="game-card back face" data-identifier="back">
...
function game_card_select(card_game) {
    ...
    const turn_down = card_game.querySelector(".back");
    const turn_down2 = card_game.querySelector(".front");
    ...
    if(counter_move === 0){
        ...
        let play_pause_button_visibility = document.querySelector(".button");
    ...
    if(counter === 1){
        attempt1 = card_game.querySelector(".back");
        attempt1f = card_game.querySelector(".front");
        ...
        attempt2 = card_game.querySelector(".back");
        attempt2f = card_game.querySelector(".front");
...
function end_game() {
    ...
    if(cards_down.length === number_of_cards){
        ...
        while(question_counter === 0){
            let question = prompt(" Deseja jogar novamente?");
            ...
            if(question === "Sim"){
                ...
                let restart_game_button = document.querySelector(".reiniciar");
                restart_game_button.classList.add("restart-game-visible");

                let play_pause_button = document.querySelector(".game-buttons .button");
                play_pause_button.classList.remove("button");
...
function play_pause() {
    let play_pause_button = document.querySelector(".button");
